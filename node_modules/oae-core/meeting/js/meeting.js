                        ...
                        oae.api.meeting.getRecording(meeting.id, function(err, info) {
                            if ( !err  && info.returncode == 'SUCCESS' && info.recordings.length > 0) {
                                ...
                                for (var i=0; i<info.recordings.length; i++ ) {
                                    var meeting_recordings_table_tbody_tr = $('<tr id="recording_"' + info.recordings[i].recordID + '"></tr>');
                                    var html_meeting_recordings_table_tbody_tr = '<td>'+ i +'</td><td>'+ info.recordings[i].name +'</td><td>'+ info.recordings[i].startTime +'</td><td>'+ info.recordings[i].endTime +'</td>';
                                    if ( meeting.isManager ) {
                                        var published = info.recordings[i].published;
                                        var publish_name = published == 'true' ? 'unpublish' : 'publish'
                                        html_meeting_recordings_table_tbody_tr += '<td><button class="publish-recording" data-published="' + published + '" data-id="' + info.recordings[i].recordID + '">' + publish_name + '</td>';
                                    }
