    ...
    return function(uid, showSettings, widgetData) {
        ...
        var renderMeeting = function(meeting) {
            ...
            oae.api.meeting.infoMeeting(meeting.id, function(err, info) {
                var meeting_status;
                if (!err) {
                    if ( meeting.isManager || meeting.canPost ) {
                        ...
                        if ( meeting.isManager || meeting.allModerators == 'true' || info.returncode === 'SUCCESS' || meeting.waitModerator == 'false' ) {
                        ...
                        if ( info.returncode === 'SUCCESS' && (meeting.isManager || meeting.allModerators == 'true') ) {
                        ...
                        ////Recordings should be shown only if enabled and if there is any
                        var recording_enabled = oae.api.config.getValue('oae-bbb', 'bbb', 'recording');
                        if ( recording_enabled ) {
                            oae.api.meeting.getRecording(meeting.id, function(err, info) {
                                if ( !err  && info.returncode == 'SUCCESS' && info.recordings.length > 0) {
                                    var t_headers = ["Name", "Started", "Ended"];
                                    if ( meeting.isManager ) {
                                        t_headers.push("Toolbar");
                                    }
                                    var meeting_recordings = $('<div><h3>Recordings</h3><br></div>');
                                    ...
                                    for (var i=0; i<info.recordings.length; i++ ) {
                                        ...
                                        if ( info.recordings[i].published === 'true' ) {
                                            hidden = '';
                                        }
                                        var playback_links = '<div class="playback-links" ' + hidden + '>';
                                        var playbacks = [];
                                        ...
                                        var html_meeting_recordings_table_tbody_tr = '<td>' + info.recordings[i].name + playback_links + '</td><td>'+ Date(info.recordings[i].startTime) +'</td><td>'+ Date(info.recordings[i].endTime) +'</td>';
                                        if ( meeting.isManager ) {
                                            var published = info.recordings[i].published;
                                            var publish_name = published == 'true' ? 'unpublish' : 'publish'
                                            html_meeting_recordings_table_tbody_tr += '<td><button class="publish-recording btn btn-default btn-xs" data-published="' + published + '">' + publish_name + '</button>';
                                            html_meeting_recordings_table_tbody_tr += '<img class="processing-indicator" title="Processing" alt="Processing" src="/node_modules/oae-core/meeting/img/processing16.gif" style="display: none;">';
                                            html_meeting_recordings_table_tbody_tr += '<button class="oae-trigger-deleterecording btn btn-danger btn-xs" data-id="' + info.recordings[i].recordID + '">delete</button></td>';
                                        }
                                    ...
                                    $('#meeting-recordings', $rootel).html(meeting_recordings);
                                }
                            });
                        }
        ...
        var initMeeting = function() {
            ...
            $(document).one('oae.editmeeting.done', function(ev, data) {
                console.info('Edit meeting done');
                renderMeeting(data);
            });

            $(document).one('oae.trigger.managemeeting-refresh', function(ev, data) {
                console.info('Executing management refresh');
                renderMeeting(data);
            });

            $(document).on('oae.deleterecording.done', function(ev, data) {
                $('#recording_' + data.recordingId).remove();
            });
