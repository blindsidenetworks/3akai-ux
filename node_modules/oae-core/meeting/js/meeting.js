                        ...
                        oae.api.meeting.getRecording(meeting.id, function(err, info) {
                            if ( !err  && info.returncode == 'SUCCESS' && info.recordings.length > 0) {
                                ...
                                for (var i=0; i<info.recordings.length; i++ ) {
                                    ...
                                    var playback_links = '';
                                    var playbacks = [];
                                    if (Array.isArray(info.recordings[i].playback)) {
                                      playbacks = info.recordings[i].playback;
                                    } else {
                                      playbacks.push(info.recordings[i].playback);
                                    }
                                    playbacks.forEach(function(element, index, array) {
                                      playback_links += ('<a target="_blank" href="' + element.format.url + '">' + element.format.type + '</a>');
                                    });
